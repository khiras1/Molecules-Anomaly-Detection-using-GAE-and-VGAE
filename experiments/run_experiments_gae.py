...
from main_gae import pretrain_model_on_enzymes, finetune_on_mutag_anomaly, classification_task
...
def run_experiments(name):
    """
    Run grid search experiments over hyperparameters for GAE pretraining,
    ...
    for lr, batch_size, hidden_channels, latent_dim, dropout in tqdm(param_combinations, desc="Eksperymenty"):
        ...
        name = f"GAE_lr_{lr}_batch_{batch_size}_hidden_{hidden_channels}_latent_{latent_dim}_dropout_{dropout}"
    ...
    csv_file = f"./results/results_GAE.csv"
    ...
    for metric in metrics:
        ...
        # Build name
        name = f"BEST_GAE_{metric}_lr_{lr}_batch_{batch_size}_hidden_{hidden_channels}_latent_{latent_dim}_dropout_{dropout}"
    ...
    best_csv_file = f"./results/best_results_GAE.csv"
