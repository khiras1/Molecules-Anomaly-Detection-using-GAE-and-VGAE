...
from train_model import train_model_vgae
from vgae_encoder import VGAEEncoder
...
def create_model(in_channels, hidden_channels, latent_dim, dropout, lr):
    """
    Create and initialize a Variational Graph Autoencoder (VGAE) model and its optimizer.
    Args:
    ...
    Returns:
        tuple: (model, optimizer) where model is a VGAE instance and optimizer is Adam.
    """
    set_seed()
    encoder = VGAEEncoder(in_channels, hidden_channels, latent_dim, dropout).to(device)
...
def pretrain_model_on_enzymes(
    ...
    """
    Pretrain the VGAE model on the ENZYMES dataset.
    Args:
    ...
    train_model_vgae(loader, model, optimizer, epochs, name)
    return model
...
def finetune_on_mutag_anomaly(
    ...
    """
    Fine-tune or train from scratch the VGAE model on MUTAG for anomaly detection.
    Trains on normals, then evaluates reconstruction and embedding-based anomaly detection using GMM.
    Args:
        model (VGAE or None): Pretrained model to fine-tune. If None, trains from scratch.
    ...
    gmm_eval.fit_gmm(n_components=1)
...
def classification_task(
    ...
    Trains VGAE if needed, extracts embeddings, and runs a downstream classifier.
    Args:
        ...
        pretrained_model (VGAE, optional): Pretrained VGAE encoder for embeddings.
        hidden_channels (int): Hidden channels if training VGAE from scratch.
        latent_dim (int): Latent dimension if training from scratch.
        dropout (float): Dropout rate if training from scratch.
        lr (float): Learning rate.
        batch_size (int): Batch size.
        epochs (int): Training epochs for VGAE.
